...
class Select extends React.Component {
  ...
  handleBackfill = item => {
    if (
      !this.props.backfill ||
      !(isSingleMode(this.props) || isCombobox(this.props))
    ) {
      return
    }
      ...
      if (input && (open || isMultipleOrTagsOrCombobox(this.props))) {
        if (activeElement !== input) {
          if (
            !(
              window.retailAdmin &&
              window.retailAdmin.antdSelectMultipleAutoFocusDisabled
            )
          ) {
            input.focus()
            this._focused = true
    ...
    React.Children.forEach(children, child => {
      ...
      if (child.type.isSelectOptGroup) {
        ...
        if (inputValue && this.filterOption(inputValue, child)) {
          const innerItems = childrenToArray(child.props.children).map(
            subChild => {
  ...
  renderTopControlNode = () => {
    ...
    if (isSingleMode(props)) {
      ...
      if (maxTagCount !== undefined && value.length > maxTagCount) {
        ...
        const omittedValues = this.getVLForOnChange(
